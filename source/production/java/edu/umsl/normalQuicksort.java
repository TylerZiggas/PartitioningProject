...
then finally uses a normal Quicksort method to sort the array.
...
public class normalQuicksort {
    ...
    public static void quickSort(int[] quickSortArray, int left, int right) {
        if (left < right) { // Check to see if we need to do more quicksort
            int split = partitionNormal(quickSortArray,left,right);
            quickSort(quickSortArray,left,split);
            quickSort(quickSortArray,split+1,right);
        }
    }

    public static int partitionNormal(int[] quickSortArray, int left, int right) { // Hoare's Partitioning with the leftmost element
    ...
    public static void main (String args[]) {
        ...
        long normalStart = System.nanoTime(); // Start timer
        quickSort(quickSortArray, left, right); // The type of quicksort used is called here
        System.out.println("Normal Quicksort Time: " + (System.nanoTime() - normalStart) + "ns");
    }
}